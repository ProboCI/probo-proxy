...
var defaultActivatingHtml =
  '<h1>Your build is activating</h1>' +
  '<p>This build is starting up and will be ready shortly. ' +
  'This page will refresh automatically.</p>';

// The container is not answering health checks yet. Tell the client to come
// back: browsers get an auto-refreshing HTML page, everything else gets a 503
// (JSON or plain text) with a Retry-After header.
function respondWithActivating(req, res, config) {
  ...
  if (wantsHtml(req)) {
    var body = config.activatingHtml || defaultActivatingHtml;
    var html =
      '<!DOCTYPE html><html><head>' +
      '<meta charset="utf-8">' +
      '<meta http-equiv="refresh" content="' + refresh + '">' +
      '<title>Build activating</title>' +
  ...
  if (accepts(req, 'application/json')) {
    ...
    return res.end(JSON.stringify({ status: 'activating', message: 'Build is activating' }));
  }

  res.writeHead(503, { 'Retry-After': refresh });
  res.end('Build is activating\n');
}
...
function setupServer(config) {
  var server = http.createServer(function (req, res) {
    ...
    proxyLookup(dest, { log: req.log }, function (err, result) {
      ...
      if (wantsHtml(req)) {
        // Browser navigation: probe the container. If it isn't ready, show the
        // auto-refreshing "activating" screen instead of proxying to a dead port.
        healthCheck.check(
          target,
        ...
        // Non-HTML (assets, API): don't pay for a probe. Route only if a recent
        // HTML navigation already marked this target healthy; otherwise 503.
        if (healthCheck.isKnownHealthy(target.url)) {
          return proxyToTarget();
        }
        return respondWithActivating(req, res, config);
      }
