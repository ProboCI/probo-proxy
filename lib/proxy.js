...
// The request's base URL is black-holed. By default just close the connection
// without answering (no bytes spent on a bot); with rateLimitAction '429' send
// a bare 429 so a human caught in the block can tell what happened.
function blackHole(req, res) {
  if (rateLimit.getAction() === '429') {
    if (res.headersSent) return;
    res.writeHead(429, {
      'Retry-After': rateLimit.getBlockSeconds(),
      'Connection': 'close',
    });
    return res.end();
  }
...
function setupServer(config) {
  var server = http.createServer(function (req, res) {
    ...
    // Rate limit gate: runs before anything else is allocated for the request
    // so a black-holed base URL costs one map lookup and nothing more — no
    // request id, no logger, no dest parse, and above all no lookup call.
    if (rateLimit.isEnabled()) {
      var verdict = rateLimit.hit(req);
      if (verdict.blocked) {
        // Log the trip once, not every dropped request after it.
        if (verdict.tripped) {
          log.warn(
            {
              key: verdict.key,
            ...
            'rate limit tripped; black-holing base url'
          );
        }
