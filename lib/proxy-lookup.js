...
function lookup(dest, opts, cb){
  ...
  // currently, only the build id dest is supported
  if(!dest.build){
    return cb(Error("build id required in destination identifier, have: " + JSON.stringify(dest)))
  }

  var buildId = dest.build
  ...
  if(config.cacheEnabled){
    ...
    if(cached){
      log.debug("proxy lookup cache HIT")

      return cb(null, cached)
    }
  }
  log.debug("proxy lookup cache MISS")
